(function () {
  ...
  const DEFAULT_SECTION_TEXTS = {
    ...
    termsText:
      "כלל התוצרים יימסרו בפורמטים דיגיטליים בלבד; הצעת המחיר אינה כוללת הוצאה לאור.\nכלל ההתנהלות בפרויקט תהיה מול גורם אחד אשר יוגדר כ-POC וינהל את כל תחום העברת הידע מולנו; דרכו יתקבלו אצלנו ויימסרו אצלכם חומרים.\nהעבודה מותנית בקבלת הזמנת עבודה רשמית לפחות 20 ימי עסקים טרם תחילת העבודה בפועל.\nImprove-IT אינה אחראית בגין כל אי התאמה או נזק, בין ישיר ובין עקיף, שייגרם כתוצאה משימוש בתוכנה או בתוכן על ידי הלקוח; מוסכם כי השימוש בתוכן ובתוכנה, ככל שיירכשו, הינו על אחריותו הבלעדית של הלקוח.\nסך החבות המצטברת לתשלום פיצויים בגין נזקים, מכל עילה שהיא (נזיקית, חוזית ואחרת), לא יעלה על סכום התמורה.\nהחברה רשאית לעשות שימוש בתוצרים ובחומריהם.",
    cancellationText:
      "על תכולה אשר תבוטל לפני תחילת העבודה הרשמית ייגבו 30% מסך מחיר התכולה המבוטלת (היקף הביטול), למעט אם הועברו חומרים כלשהם למזמין; במקרה כזה תשולם העלות המלאה.\nתכולת עבודה אשר נכנסה לעבודה תשולם בהתאם לאבן הדרך הבאה בתוספת 20% מסך שארית ההזמנה (היקף הביטול).\nלא יתקיימו החזרים מאבני דרך ששולמו.\nכלל תכולות העבודה משפיעות האחת על השנייה; ביטול של תכולות עלול לגרור שינוי במחירים ליחידה של שאר התכולות המוזמנות, בכפוף לאמור בהצעה זו ולשיקול דעתה הבלעדי של Improve-IT.",
  };
  const LEGACY_DEFAULT_SECTION_TEXTS = {
    termsText:
  ...
  function normalizeQuote(raw, options = {}) {
    ...
    Object.entries(DEFAULT_SECTION_TEXTS).forEach(([key, defaultValue]) => {
      ...
      if (isLegacyDefaultSectionText(key, merged[key])) {
  ...
  function isLegacyDefaultSectionText(key, value) {
    const legacyValue = LEGACY_DEFAULT_SECTION_TEXTS[key];
    if (!legacyValue) return false;
    return normalizeTextForDefaultMigration(value) === normalizeTextForDefaultMigration(legacyValue);
  }

  function normalizeTextForDefaultMigration(value) {
